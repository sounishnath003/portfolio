      ...
      <div class="my-4">
        <form [formGroup]="form" (ngSubmit)="onSubmitClicked()">
          <div class="flex flex-col space-y-3">
            ...
            <input required type="url" name="logo" id="logo" formControlName="logo"
          ...
          <div class="flex flex-col space-y-3 my-3">
            ...
            <input required type="text" name="projectType" id="projectType" formControlName="projectType"
                   class="py-2 px-4 rounded bg-gray-50 border outline-none border-gray-600"
                   placeholder="Project Type: Angular">
          </div>
          <div class="flex flex-col space-y-3">
            <label for="tags">Tags: </label>
            <input required type="text" name="tags" id="tags" formControlName="tags"
                   class="py-2 px-4 rounded bg-gray-50 border outline-none border-gray-600"
                   placeholder="#angular, #webdev, ...">
          </div>
          <div class="flex flex-col space-y-3 my-3">
            <label for="description">Description: </label>
            <textarea required="true" rows="5" type="text" name="description" id="description"
...
export class AddFormComponent implements OnInit, OnDestroy {
  ...
  constructor(private readonly projectTypeService: ProjectTypeService, private router: Router) {
    this.preloadChecksForUrlState();
  }

  ngOnInit(): void {
    if (this.projectToEdit !== null) {
      this.form.setValue({...this.form.value, ...this.projectToEdit, tags: this.projectToEdit.tags.toString()});
  ...
  private createNewProjectTypeRecord(rawFormData: any) {
    ...
    this.activeSubscription = this.projectTypeService.createNewProjectType(formData).subscribe(
      (data) => {
  ...
  private editProjectType(rawFormData: any) {
    ...
    this.activeSubscription = this.projectTypeService.updatedProjectType(formData).subscribe(
      (data) => {
        this.router.navigate(['cms', 'dashboard', 'project-type']);
        this.form.reset();
      },
