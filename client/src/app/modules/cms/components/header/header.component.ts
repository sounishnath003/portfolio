  ...
  template: `
    <div class="mb-6">
      <div class="p-4 rounded mx-auto">
        <div class="text-2xl text-center font-semibold text-blue-600">
          CMS Dashboard Manager
        </div>
        <nav
          class=" flex my-4 flex-wrap flex-row justify-center items-center space-x-10"
        >
          <div
            *ngFor="let link of links"
            class="cursor-pointer"
            [routerLink]="link.routeTo"
            routerLinkActive="bg-gray-50 font-semibold p-2 rounded text-blue-700"
          >
            {{ link.linkName }}
          </div>
        </nav>
