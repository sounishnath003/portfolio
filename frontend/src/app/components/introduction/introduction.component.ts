...
import { map, Observable, repeat, timer } from 'rxjs';
...
export class IntroductionComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.featureObserver$ = timer(0, 2800).pipe(
      map(() => {
        const d = '' + this.features.shift();
